...
TimeSyncClient::TimeSyncClient(const std::string &root, const std::string &server) {
	...
	// create channel arguments
  	...
  	channelArgs.SetSslTargetNameOverride("timesync.spot.robot"); // put into kv map later
  	stub_ = TimeSyncService::NewStub(grpc::CreateCustomChannel(server, grpc::SslCredentials(opts), channelArgs));
}
...
//       reply = getTimeSyncUpdate(request, clock_identifier);
