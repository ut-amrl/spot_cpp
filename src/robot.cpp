...
Robot::Robot(const std::string &name, const std::string &username, const std::string &password) : 
    ...
    _robotCommandClientPtr(std::shared_ptr<RobotCommandClient>(new RobotCommandClient(_directoryClientPtr->getEntry(ROBOT_COMMAND__CLIENT_NAME).service_entry().authority(), _token)));
    _robotIdClientPtr(std::shared_ptr<RobotIdClient>(new RobotIdClient(_directoryClientPtr->getEntry(ROBOT_ID_CLIENT_NAME).service_entry().authority(), _token)));
