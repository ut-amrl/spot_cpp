...
void SpotROSNode::run(int argc, char **argv, const std::string &username, const std::string &password) {
    ...
    ros::Publisher image_back_pub = nh.advertise<sensor_msgs::Image>("spot/camera/back/image", 10);
    ros::Publisher image_frontleft_pub = nh.advertise<sensor_msgs::Image>("spot/camera/frontleft/image", 10);
    ros::Publisher image_frontright_pub = nh.advertise<sensor_msgs::Image>("spot/camera/frontright/image", 10);
    ros::Publisher image_left_pub = nh.advertise<sensor_msgs::Image>("spot/camera/left/image", 10);
    ros::Publisher image_right_pub = nh.advertise<sensor_msgs::Image>("spot/camera/right/image", 10);

    // depth
    ros::Publisher depth_back_pub = nh.advertise<sensor_msgs::Image>("spot/depth/back/image", 10);
    ros::Publisher depth_frontleft_pub = nh.advertise<sensor_msgs::Image>("spot/depth/frontleft/image", 10);
    ros::Publisher depth_frontright_pub = nh.advertise<sensor_msgs::Image>("spot/depth/frontright/image", 10);
    ros::Publisher depth_left_pub = nh.advertise<sensor_msgs::Image>("spot/depth/left/image", 10);
    ros::Publisher depth_right_pub = nh.advertise<sensor_msgs::Image>("spot/depth/right/image", 10);

    // image camera info
    ros::Publisher image_back_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/camera/back/camera_info", 10);
    ros::Publisher image_frontleft_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/camera/frontleft/camera_info", 10);
    ros::Publisher image_frontright_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/camera/frontright/camera_info", 10);
    ros::Publisher image_left_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/camera/left/camera_info", 10);
    ros::Publisher image_right_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/camera/right/camera_info", 10);

    // depth camera info
    ros::Publisher depth_back_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/depth/back/camera_info", 10);
    ros::Publisher depth_frontleft_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/depth/frontleft/camera_info", 10);
    ros::Publisher depth_frontright_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/depth/frontright/camera_info", 10);
    ros::Publisher depth_left_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/depth/left/camera_info", 10);
    ros::Publisher depth_right_camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("spot/depth/right/camera_info", 10);

    // status (handled by robotstate)
    ros::Publisher joint_state_pub = nh.advertise<sensor_msgs::JointState>("spot/joint_states", 10);
    ros::Publisher metrics_pub = nh.advertise<spot_msgs::Metrics>("spot/status/metrics", 10);
    ros::Publisher leases_pub = nh.advertise<spot_msgs::LeaseArray>("spot/status/leases", 10);
    ros::Publisher odometry_twist_pub = nh.advertise<geometry_msgs::TwistWithCovarianceStamped>("spot/odom/twist", 10);
    ros::Publisher odometry_pub = nh.advertise<nav_msgs::Odometry>("spot/odom", 10);
    ros::Publisher feet_pub = nh.advertise<spot_msgs::FootStateArray>("spot/status/feet", 10);
    ros::Publisher estop_pub = nh.advertise<spot_msgs::EStopStateArray>("spot/status/estop", 10);
    ros::Publisher power_pub = nh.advertise<spot_msgs::PowerState>("spot/status/power_state", 10);
    ros::Publisher battery_pub = nh.advertise<spot_msgs::BatteryStateArray>("spot/status/battery_states", 10);

    // feedback (handled by robot id, sdk)
    ros::Publisher feedback_pub = nh.advertise<spot_msgs::Feedback>("spot/status/feedback", 10);

    // mobility params (handled by sdk)
    ros::Publisher mobility_params_pub = nh.advertise<spot_msgs::MobilityParams>("spot/status/mobility_params", 10);
    ...
    ros::ServiceServer sit_srv = nh.advertiseService("spot/sit", &SpotROSNode::sit_srv, this);
    ros::ServiceServer stand_srv = nh.advertiseService("spot/stand", &SpotROSNode::stand_srv, this);
    ros::ServiceServer power_on_srv = nh.advertiseService("spot/power_on", &SpotROSNode::power_on_srv, this);
    ros::ServiceServer power_off_srv = nh.advertiseService("spot/power_off", &SpotROSNode::power_off_srv, this);
    ...
    while (ros::ok()) {        
        ...
        if (update_ready("state")) {
            ...
            odometry_pub.publish(get_odom_from_state(state, false));
